...
import logging
import json
from bshopper_app.models import UserPreferences, MailChimp

from chimpusers.models import UserSubscription
import datetime
from django.core.mail import send_mail

from api import FacebookUserConverter
...
def connect(request, graph):
    ...
    try:
        ...
        #added by Ivan for user preferences
        converter = FacebookUserConverter(graph)
        converter.get_and_store_likes(request.user)
        ...
        user_info_json = json.loads(request.user.raw_data)
        try:
            location = user_info_json["location"]["name"]
        except:
            location = ""
        is_welcome_email = False
        if len(userpreferences) > 0:
            userpreference = userpreferences[0]
            #userpreference.FirstName = request.user.first_name
            #userpreference.LastName = request.user.last_name
            #userpreference.Gender = request.user.gender
            #userpreference.BirthDay = request.user.date_of_birth
            #userpreference.ProfilePicturePath = request.user.image
            ...
            new_userPreference = UserPreferences.objects.create(
                ...
                ProfilePicturePath = request.user.image,
                Location = location,
             )
        ...
        if len(mailchimps) > 0:
            ...
            mailchimp.MailChimpAllowed = True
            mailchimp.save()
        else:
            new_mailchimp = MailChimp.objects.create(
                user = request.user,
                MailChimpAllowed = True,
             )

        #subscribe mailchimp
        #subscribe mailchimp
        subscription = UserSubscription.objects.get(user=request.user)
        birthday = request.user.date_of_birth.strftime('%Y-%m-%d')
        ...
        if is_welcome_email:
            #by Ivan
            subject = "Welcome to Bored Shooper!"
            message = "Hi %s,Welcome to Bored Shooper.\nThanks, welcome you." % request.user.first_name
